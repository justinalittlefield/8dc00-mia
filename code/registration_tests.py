...
def mutual_information_test():
    I = plt.imread('../data/cameraman.tif')
    I1 = np.random.randint(255, size=(512, 512))
    J1 = np.random.randint(255, size=(512, 512))

    # mutual information of an image with itself
    p1 = reg.joint_histogram(I, I)
    MI1 = reg.mutual_information(p1)
    assert MI1 > 1, "mutual information of image with itself should be greater than 1"
    # ------------------------------------------------------------------#
    # TODO: Implement a few tests of the mutual_information definition
    # ------------------------------------------------------------------#
    # mutual information of two random images
    p2 = reg.joint_histogram(I1, J1)
    MI2 = reg.mutual_information(p2)
    assert MI2 < 0.01, "mutual information of two random images should be very low"
    # mutual information of image with itself rotated 90 degrees
    T_R = util.t2h(reg.rotate(np.pi / 2), np.zeros(2))
    p3 = reg.joint_histogram(I, reg.image_transform(I, T_R)[0])
    MI3 = reg.mutual_information(p3)
    assert MI3 > 0.001 and MI3 < 1, "mutual information of image with itself rotated should be greater than zero but not very high"
    print('Test successful!')
...
def mutual_information_e_test():
    ...
    # mutual information of an image with itself
    p = reg.joint_histogram(I, I)
    MI1 = reg.mutual_information_e(p)
    MI2 = reg.mutual_information(p)
    assert abs(
    ...
    # mutual information of two random images
    p2 = reg.joint_histogram(I1, J1)
    MI2 = reg.mutual_information_e(p2)
    assert MI2 < 0.01, "mutual information of two random images should be very low"
    # mutual information of image with itself rotated 90 degrees
    T_R = util.t2h(reg.rotate(np.pi / 2), np.zeros(2))
    p3 = reg.joint_histogram(I, reg.image_transform(I, T_R)[0])
    MI3 = reg.mutual_information_e(p3)
    assert MI3 > 0.001 and MI3 < 1, "mutual information of image with itself rotated should be greater than zero but not very high"
